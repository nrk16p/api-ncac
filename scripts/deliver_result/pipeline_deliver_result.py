...
(type=monthly-deliver-result) and stores rows in MongoDB db `mena-bi`,
...
def list_files(session, year, month=None):
    ...
    while True:
        params = {
            "type": "monthly-deliver-result", "use_as": "batch-report",
        ...
        for row in re.findall(r"<tr.*?</tr>", table.group(0), re.S):
            ...
            if file_id in files:
                continue
            text = re.sub(r"<[^>]+>", "|", row)
            caption_m = re.search(r"ผลการจัดส่ง\s*:\s*(.+?)\s+on\s+(\d{4})-(\d{2})", text)
            if not caption_m:
                continue
            ...
            files[file_id] = {
                "file_id": file_id,
                "caption": f"ผลการจัดส่ง : {branch} on {f_year}-{f_month:02d}",
